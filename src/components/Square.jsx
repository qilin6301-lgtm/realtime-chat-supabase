...
export default function Square({ profile }) {
  ...
  async function fetchPosts() {
    ...
    const { data } = await supabase
      .from('posts')
      .select(`id, content, image_urls, created_at, profiles:user_id ( id, username, gender, age, avatar_url, country )`)
      ...
      <div style={{ padding: '12px 16px', display: 'flex', gap: 8, borderBottom: '1px solid var(--border)' }}>
        {['all', 'male', 'female'].map(f => (
          <button
            ...
            onClick={() => setFilter(f)}
          >
            {f === 'all' ? t('all') : f === 'male' ? '♂ ' + t('male') : '♀ ' + t('female')}
          </button>
        ))}
      </div>
          ...
          <div key={post.id} className="card fade-in" style={{ padding: 16, marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              {post.profiles?.avatar_url ? (
                <img src={post.profiles.avatar_url} alt="" className="avatar-img" style={{ width: 36, height: 36 }} />
              ) : (
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                  <span className={`gender-tag ${post.profiles?.gender === 'male' ? 'gender-male' : 'gender-female'}`}>
                    {post.profiles?.gender === 'male' ? '♂' : '♀'} {t('years_old', { age: post.profiles?.age })}
                  </span>
                  {post.profiles?.country && <span> · {countryName(post.profiles.country)}</span>}
                  {' · '}{new Date(post.created_at).toLocaleString(undefined, { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
            </div>
            {post.content && (
              <p style={{ lineHeight: 1.5, whiteSpace: 'pre-wrap', marginBottom: post.image_urls?.length ? 10 : 0 }}>
                {post.content}
              </p>
            )}
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {post.image_urls.map((url, i) => (
                  <img
                    ...
                    src={url}
                    alt=""
                    style={{ maxWidth: '100%', maxHeight: 240, borderRadius: 10, cursor: 'pointer' }}
