...
export default function Friends({ profile, onStartChat }) {
  ...
  async function loadFriends() {
    ...
    const { data } = await supabase
      ...
      .select(`
        ...
        friend:friend_id ( id, username, gender, age )
      `)
  ...
  async function handleSearch(e) {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('id, username, gender, age, is_approved')
  ...
  async function startChatWith(target) {
    ...
    // 查找或创建会话（保证 user1_id < user2_id 避免重复）
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
          ...
          {results.map(user => (
            <div key={user.id} style={{
              ...
              <button className="btn btn-ghost" style={{ padding: '6px 10px', fontSize: 12 }} onClick={() => addFriend(user)}>
                加好友
              </button>
              <button className="btn btn-primary" style={{ padding: '6px 10px', fontSize: 12 }} onClick={() => startChatWith(user)}>
                聊天
              </button>
            </div>
          ))}
